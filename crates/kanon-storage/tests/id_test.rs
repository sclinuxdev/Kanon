...
fn test_plugin_id_invalid_cases() {
    ...
    assert_eq!("   ".parse::<PluginId>(), Err(PluginIdError::Empty));
    assert_eq!(".".parse::<PluginId>(), Err(PluginIdError::ReservedPathToken));
    assert_eq!("..".parse::<PluginId>(), Err(PluginIdError::ReservedPathToken));
    assert_eq!("a/b".parse::<PluginId>(), Err(PluginIdError::ContainsPathSeparator));
    assert_eq!("a\\b".parse::<PluginId>(), Err(PluginIdError::ContainsPathSeparator));
    assert_eq!("a\0b".parse::<PluginId>(), Err(PluginIdError::ContainsNullByte));
    assert_eq!("a\nb".parse::<PluginId>(), Err(PluginIdError::InvalidCharacters));
    assert_eq!(" foo".parse::<PluginId>(), Err(PluginIdError::InvalidCharacters));
    assert_eq!("foo ".parse::<PluginId>(), Err(PluginIdError::InvalidCharacters));
}
...
fn test_plugin_id_serde() {
    ...
    let deserialized: PluginId = serde_json::from_str(&json).expect("Failed to deserialize PluginId");
